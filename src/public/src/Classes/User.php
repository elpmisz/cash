...
class User
{
  ...
  {
    $sql = "SELECT COUNT(*) FROM cash.login WHERE email = ?";
  ...
  {
    $sql = "SELECT status FROM cash.login WHERE email = ?";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute($data);
  ...
  {
    $sql = "SELECT password FROM cash.login WHERE email = ?";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute($data);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
  ...
  {
    $sql = "SELECT password_default FROM cash.system WHERE id = 1";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
  ...
  public function login_insert($data)
  {
    ...
    $sql = "INSERT INTO cash.login(uuid,email,password) VALUES(uuid(),?,?)";
    $stmt = $this->dbcon->prepare($sql);
  ...
  {
    $sql = "SELECT COUNT(*) FROM cash.login";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
