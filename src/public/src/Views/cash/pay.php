...
<div class="card shadow">
  ...
  <div class="card-body">
    ...
    <form action="/cash/pay" method="POST" class="needs-validation" novalidate>
      <div class="row">
      ...
      <div class="row mb-2">
        <label class="col-xl-2 ml-5 col-form-label">การจ่ายเงิน</label>
        <div class="col-xl-8">
          <div class="form-group pl-3 pt-2">
            <label class="form-check-label px-3">
              <input class="form-check-input" type="radio" name="pay_type" value="1" required>
              <span class="text-success">เงินสด</span>
            </label>
            <label class="form-check-label px-3">
              <input class="form-check-input" type="radio" name="pay_type" value="2" required>
              <span class="text-primary">โอนเงิน</span>
            </label>
            <label class="form-check-label px-3">
              <input class="form-check-input" type="radio" name="pay_type" value="3" required>
              <span class="text-danger">เช็ค (Cheque)</span>
            </label>
          </div>
        </div>
      </div>
      <div class="row mb-2">
        <label class="col-xl-2 ml-5 col-form-label">เลขที่เช็ค (Cheque)</label>
        <div class="col-xl-4">
      ...
      <div class="row mb-2">
        ...
        <div class="col-xl-2">
          <input type="number" class="form-control form-control-sm text-right" name="payment" step="0.01" required>
          <div class="invalid-feedback">
            กรุณา กรอกข้อมูล!
          </div>
        </div>
...
<script>
  $(document).on("click", "input[name='pay_type']", function() {
    let pay = ($(this).val() ? parseInt($(this).val()) : "");
    if (pay === 3) {
      $("input[name='cheque_number']").prop("required", true);
    } else {
      $("input[name='cheque_number']").prop("required", false);
    }
  });
</script>